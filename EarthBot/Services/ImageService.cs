...
namespace EarthBot.Services
{
    public class ImageService
    {
        ...
        private LocatedObject<string> GeneratePictureUrl()
        {
            ...
            return new LocatedObject<string>(request.GetPostRequestUrl(), latitude, longitude, request.ZoomLevel);
        }
        ...
        public async Task<LocatedObject<Image<Rgba32>>> GetPostableImage()
        {
            while (true)
            {
                ...
                do
                {
                    ...
                    picture = await DownloadImage(url.Object);
                } while (picture == null);

                CropImage(picture);
                return new LocatedObject<Image<Rgba32>>(picture, url.Latitude, url.Longtitude, url.Zoom);
            }
        }
